...
// ─── AskUserQuestion parameter schema (Claude Code format) ──────────────────
...
export default function (pi: ExtensionAPI) {
	...
	// ── AskUserQuestion (Claude Code shim) ───────────────────────────────────
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			if (!ctx.hasUI) {
				return {
					...
					details: { question: params.question, options: params.options, answer: null, wasCustom: false } as AskUserQuestionDetails,
				};
			}

			// Build display options: user-provided strings + "Type something..." fallback
			...
			return {
				content: [{ type: "text", text: result.wasCustom ? `User wrote: ${result.answer}` : `User selected: ${result.answer}` }],
				details: { question: params.question, options: params.options, answer: result.answer, wasCustom: result.wasCustom } as AskUserQuestionDetails,
			};
		},

		renderCall(args, theme) {
			const text = theme.fg("toolTitle", theme.bold("AskUserQuestion ")) + theme.fg("muted", args.question);
			return new Text(text, 0, 0);
		},
