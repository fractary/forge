...
Where:
- `template_path`: Path to chosen template (e.g., `plugins/forge/templates/agent/manager.md.template`)
...
1. Run XML markup validator:
   ```bash
   plugins/forge/validators/xml-validator.sh {output_path} agent
   ```

2. Run frontmatter validator:
   ```bash
   plugins/forge/validators/frontmatter-validator.sh {output_path} agent
   ```
...
**Uses:**
- Templates: `plugins/forge/templates/agent/*.template`
- Validators: `plugins/forge/validators/*.sh`
- Scripts: `plugins/forge/skills/*/scripts/*.sh`
