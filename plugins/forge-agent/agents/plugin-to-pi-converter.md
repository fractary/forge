...
The conversion rules are defined in the `fractary-forge-agent:conversion-mapping` skill — read it at the start of every conversion to ensure you apply the current mapping rules.

You use four converter skills:
- `fractary-forge-agent:command-to-prompt` — converts each Claude command to a pi prompt
- `fractary-forge-agent:agent-to-skill` — converts each Claude agent to a pi skill
- `fractary-forge-agent:skill-to-skill` — converts each Claude skill to a pi skill
- `fractary-forge-agent:manifest-converter` — converts plugin.json to package.json
 ...
 *
 * AUTO-GENERATED by fractary-forge-agent:convert-plugin
