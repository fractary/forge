...
The conversion rules are defined in the `fractary-pi:conversion-mapping` skill — read it at the start of every conversion to ensure you apply the current mapping rules.

You use four converter skills:
- `fractary-pi:command-to-prompt` — converts each Claude command to a pi prompt
- `fractary-pi:agent-to-skill` — converts each Claude agent to a pi skill
- `fractary-pi:skill-to-skill` — converts each Claude skill to a pi skill
- `fractary-pi:manifest-converter` — converts plugin.json to package.json
 ...
 *
 * AUTO-GENERATED by fractary-pi:convert-plugin
